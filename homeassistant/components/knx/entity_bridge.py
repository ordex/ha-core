"""Runtime for KNX entity bridges (bidirectional Home Assistant entity <-> KNX).

A bridge makes an existing Home Assistant entity behave like a KNX actuator: it sends
the entity state to KNX status group addresses and drives the entity via service calls
when a write telegram is received on a command group address.
"""
...
class _Channel:
    """Runtime state for a single bridge channel."""

    role: str
    definition: ChannelDefinition
    remote_value: RemoteValue
    # canonical re-encoding of a value just received from the bus, pending its echo
    suppress: DPTArray | DPTBinary | None = None

    @property
    def command_addresses(self) -> list[DeviceGroupAddress]:
        """Group addresses the bridge listens on for incoming commands."""
...
class KnxEntityBridge:
    """Bridge a Home Assistant entity to KNX group addresses bidirectionally."""
    ...
    def __init__(
        ...
        """Initialize the bridge from validated store configuration."""
        self.hass = hass
        self.xknx = xknx
    ...
    async def _async_entity_changed(self, event: Event[EventStateChangedData]) -> None:
        ...
        for channel in self._channels:
            ...
            try:
                ...
                _LOGGER.warning(
                    "Could not encode %s for KNX bridge %s: %s",
                    channel.role,
                    self.entity_id,
                    err,
                )
                continue
            if payload == channel.suppress:
                channel.suppress = None  # consume the echo of a bus-driven change
                continue
    ...
    def _telegram_received_cb(self, telegram: Telegram) -> None:
        ...
        try:
            ...
            _LOGGER.warning(
                "Could not decode incoming telegram for KNX bridge %s: %s",
                self.entity_id,
                err,
            )
            return
        ...
        if call is None:
            return
        # pre-seed echo suppression with the canonical re-encoding so the resulting
        # state change is not sent back onto the bus (see _async_entity_changed)
        if channel.remote_value.group_address is not None:
            with suppress(ConversionError):
                channel.suppress = channel.remote_value.to_knx(value)
        self.hass.async_create_task(
            self.hass.services.async_call(
                call.domain, call.service, call.data, blocking=False
            ),
            f"KNX entity bridge {self.entity_id}",
        )
